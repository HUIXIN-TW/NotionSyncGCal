...
class TokenCryptoTests(unittest.TestCase):
    ...
    def test_plaintext_fallback_without_key(self):
        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(token_crypto.decrypt_token_if_needed("plain-token"), "plain-token")
            self.assertEqual(token_crypto.encrypt_token_if_enabled("plain-token"), "plain-token")

    def test_encrypt_and_decrypt_when_key_is_configured(self):
        with (
            ...
            encrypted = token_crypto.encrypt_token_if_enabled("my-secret-token")
            self.assertTrue(encrypted.startswith("enc:v1:"))
            decrypted = token_crypto.decrypt_token_if_needed(encrypted)
    ...
    def test_non_v1_payload_raises(self):
        ...
        with (
            ...
            with self.assertRaises(token_crypto.TokenCryptoError):
                token_crypto.decrypt_token_if_needed(payload)
