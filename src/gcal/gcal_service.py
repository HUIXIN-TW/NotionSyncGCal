...
def move_gcal_event(gcal_event_id, new_gcal_calendar_id):
    pass
...
def make_event_body(notion_task):
    ...
    try:
        event_icon = notion_task["properties"][
            nt.COMPLETEICON_NOTION_NAME]["formula"]["string"]
    except:
        event_icon = "❓"
    event_name = notion_task["properties"][
        nt.TASK_NOTION_NAME]["title"][0]["text"]["content"]
    event_summary = event_icon + event_name

    # set start and end date
    event_start_date = notion_task["properties"][
        nt.DATE_NOTION_NAME]["date"]["start"]
    event_end_date = notion_task["properties"][nt.DATE_NOTION_NAME]["date"]["end"]
...
#                 try:
#                     existing_event_id = el["properties"][nt.GCAL_EVENTID_NOTION_NAME][
#                         "rich_text"
#                     ][0]["text"]["content"]
#                 except:
#                     existing_event_id = ""
...
def make_event_description(initiative, info, task_status):
