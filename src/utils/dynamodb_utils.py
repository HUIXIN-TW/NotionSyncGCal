...
import boto3
from utils.token_crypto import decrypt_token_if_needed, encrypt_token_if_enabled
...
def get_notion_token_by_uuid(uuid: str) -> str:
    ...
    if "accessToken" in item:
        item["accessToken"] = decrypt_token_if_needed(item.get("accessToken"))
    return item
...
def get_google_token_by_uuid(uuid: str) -> str:
    ...
    if "refreshToken" in item:
        item["refreshToken"] = decrypt_token_if_needed(item.get("refreshToken"))
    return item
...
def update_google_token_by_uuid(uuid: str, access_token: str, refresh_token: str, expiry_date: str, updated_at: str):
    google_tbl = _get_google_tables()
    encrypted_access_token = encrypt_token_if_enabled(access_token)
    encrypted_refresh_token = encrypt_token_if_enabled(refresh_token)
