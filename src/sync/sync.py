...
def synchronize_notion_and_google_calendar(
    ...
    try:
        ...
        if task_count > NOTION_TASK_LIMIT or event_count > NOTION_TASK_LIMIT:
            logging.warning(
                f"Task count exceeds {NOTION_TASK_LIMIT}. Sync process stopped to avoid overloading the Notion database."
            )
            sys.exit(1)
    ...
    for notion_task in notion_task_list:
        ...
        try:
            notion_gcal_cal_name = notion_task["properties"][notion_service.nt.CURRENT_CALENDAR_NAME_NOTION_NAME][
                "select"
            ]["name"]
            notion_gcal_cal_id = notion_service.nt.get_cal_id(notion_gcal_cal_name)
        except:
            logger.warning(f"Calendar name not found. Use the default calendar: {notion_service.nt.GCAL_DEFAULT_NAME}")
        ...
        try:
            notion_gcal_event_id = notion_task["properties"][notion_service.nt.GCAL_EVENTID_NOTION_NAME]["rich_text"][
                0
            ]["plain_text"]
        except:
        ...
        notion_task_name = notion_task["properties"][notion_service.nt.TASK_NOTION_NAME]["title"][0]["plain_text"]

        # Notion Task properties for sync status - the last time the task was synced between Notion and Google Calendar
        try:
            notion_gcal_sync_time = notion_task["properties"][notion_service.nt.GCAL_SYNC_TIME_NOTION_NAME][
                "rich_text"
            ][0]["plain_text"]
        except:
        ...
        if not notion_gcal_event_id and should_update_google_events:
            ...
            new_gcal_event_id = gcal_service.create_gcal_event(notion_task, notion_gcal_cal_id)
        ...
        for gcal_event in gcal_event_list:
            ...
            if notion_gcal_event_id == gcal_event_id:
                ...
                if should_update_google_events and (
                    ...
                    logger.info(f"🥐 Notion Task: Updating the event in Google Calendar for task '{notion_task_name}'")

                    if notion_gcal_cal_id == gcal_cal_id:
                        gcal_service.update_gcal_event(notion_task, notion_gcal_cal_id, notion_gcal_event_id)
                    else:
    ...
    if len(gcal_event_list) > 0 and should_update_notion_tasks:
        ...
        for gcal_event in gcal_event_list:
            logger.info(f"Google Calendar: Creating a new task in Notion for event '{gcal_event.get('summary', '')}'")

            # Google Calendar Display Name
            gcal_cal_name = notion_service.nt.get_cal_name(gcal_event.get("organizer", {}).get("email"))
