# src/tests/test_notion_service.py
...
class TestNotionService(unittest.TestCase):
    ...
    def test_get_task_by_id(self, mock_retrieve):
        # Arrange
        mock_retrieve.return_value = {'id': 'page_123', 'properties': {}}
        page_id = 'page_123'

        # Act
        result = ns.get_task_by_id(page_id)

        # Assert
        self.assertEqual(result['id'], page_id)
        mock_retrieve.assert_called_once_with(page_id=page_id)
